    ...
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "var(--bg)" }}>
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm" style={{ color: "var(--fg-muted)" }}>새 계정 만들기</p>
        </div>

        <form onSubmit={handleSubmit}
              className="rounded-2xl p-8 space-y-5"
              style={{ background: "var(--card)", border: "1px solid var(--border)", boxShadow: "var(--shadow-lg)" }}>
          <h2 className="text-xl font-semibold text-center" style={{ fontFamily: "var(--font-heading)" }}>회원가입</h2>
            ...
            <div className="relative">
              ...
              <button type="button" onClick={() => setShow(!show)}
                ...
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  ...
                  {!show && <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />}
                </svg>
              </button>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium" style={{ color: "var(--fg-muted)" }}>비밀번호 확인</label>
          ...
          <p className="text-center text-sm" style={{ color: "var(--fg-muted)" }}>
            이미 계정이 있으신가요?{" "}
            <Link href="/login" className="font-medium" style={{ color: "var(--primary)" }}>로그인</Link>
          </p>
        </form>
